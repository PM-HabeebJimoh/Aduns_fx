# HYDRA-PRIME Strict Real-Time-Only Backtest Audit

- Requested period: `2026-01-01` to `2026-07-07`
- Data directory: `data/realtime_archive`
- Strict real-time archive only: `True`
- Status: **BLOCKED**
- Generated at: `2026-07-09T08:56:18.996743+00:00`

## Verdict

No backtest/performance numbers were produced because doing so would require missing data or simulated assumptions.
...
- ohlcv_intraday_bars: Required feed is absent from manifest. Squeeze, exits, stops, and day/month coverage need intraday bars.
...
- actual_execution_fills: Required feed is absent from manifest. The user forbade simulated fills/assumptions; therefore performance needs actual historical fills.
...
| `ohlcv_intraday_bars` | Volatility squeeze and execution reference | **MISSING** | 0 |  |  | Required feed is absent from manifest. Squeeze, exits, stops, and day/month coverage need intraday bars. |
...
| `actual_execution_fills` | No-simulation performance measurement | **MISSING** | 0 |  |  | Required feed is absent from manifest. The user forbade simulated fills/assumptions; therefore performance needs actual historical fills. |

## Performance

No performance table is reported. Reporting returns, win rate, drawdown, or ROI without the required real-time archive and actual fills would violate the no-lies/no-assumptions constraint.
